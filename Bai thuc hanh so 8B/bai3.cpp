...
void check(factory nhamay[], int n){
  cout << "\nNhap ten nha may ban can tim: ";
  string temp;
  cin.ignore();
  getline(cin,temp);
  bool found=false;
  for (int i=0;i<n;i++){
    if (nhamay[i].name==temp){
      found=true;
      break;
    }
  }
  if (found==true) cout << temp << " co trong danh sach nha may.\n";
  else cout << temp << " khong co trong danh sach nha may.\n";
}
